...
//         <p className="text-lg font-semibold">Çalışma Saatleri Yönetimi</p>
//       </div>
...
const HoursEditor = ({ workingHours }) => {
  const [hoursData, setHoursData] = useState([]);
  const [originalData, setOriginalData] = useState([]); // Orijinal veriyi saklamak için
  const { submitHours, loading, error } = useSubmitHours();
  const [lastSubmitTime, setLastSubmitTime] = useState(0);
  ...
  // Veri değişikliğini kontrol eden fonksiyon
  const hasDataChanged = () => {
    if (originalData.length !== hoursData.length) return true;

    return originalData.some((original) => {
      const current = hoursData.find((item) => item.day === original.day);
      if (!current) return true;

      return (
        original.openTime !== current.openTime ||
        original.closeTime !== current.closeTime ||
        original.isClosed !== current.isClosed
      );
    });
  };
  ...
  useEffect(() => {
    ...
    if (
      ...
      formattedData = workingHours.map((item) => {
        const openTime = formatTimeValue(item.openTime);
        const closeTime = formatTimeValue(item.closeTime);
    ...
    setHoursData(formattedData);
    // Orijinal veriyi de sakla (deep copy)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setOriginalData(JSON.parse(JSON.stringify(hoursData)));
    } catch (error) {
      console.error("Kaydetme hatası:", error);
      toast.error("Kaydetme sırasında bir hata oluştu.");
    }
